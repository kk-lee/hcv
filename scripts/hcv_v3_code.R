...
dat.ratio <- read.csv("data_raw/hcv.ratio.v2.csv",header = T,na.strings = "")
...
rr371 <- rma(yi = dat.ratio371$lgrr,sei = dat.ratio371$se, method = "FE") 
rr371.exp <- predict(rr371, transf = exp, digits=4)
rr371.sim <- exp(rnorm(num_iter, as.vector(rr371$b), rr371$se))

est371 <- (quantile(rr371.sim, probs = c(0.025,0.5, 0.975)))
est371 <- as.data.frame(est371)
est371 <- t(est371)

#study 671
dat.ratio671 <- subset(dat.ratio,dat.ratio$study.id==671)
rr671 <- rma(yi = dat.ratio671$lgrr,sei = dat.ratio671$se, method = "FE") 
rr671.exp <- predict(rr671, transf = exp, digits=4)
rr671.sim <- exp(rnorm(num_iter, as.vector(rr671$b), rr671$se))

est671 <- (quantile(rr671.sim, probs = c(0.025,0.5, 0.975)))
est671 <- as.data.frame(est671)
est671 <- t(est671)

#study 1131
dat.ratio1131 <- subset(dat.ratio,dat.ratio$study.id==1131)
rr1131 <- rma(yi = dat.ratio1131$lgrr,sei = dat.ratio1131$se, method = "FE") 
rr1131.exp <- predict(rr1131, transf = exp, digits=4)
rr1131.sim <- exp(rnorm(num_iter, as.vector(rr1131$b), rr1131$se))

est1131 <- (quantile(rr1131.sim, probs = c(0.025,0.5, 0.975)))
est1131 <- as.data.frame(est1131)
est1131 <- t(est1131)

#study 1146
dat.ratio1146 <- subset(dat.ratio,dat.ratio$study.id==1146)
rr1146 <- rma(yi = dat.ratio1146$lgrr,sei = dat.ratio1146$se, method = "FE") 
rr1146.exp <- predict(rr1146, transf = exp, digits=4)
rr1146.sim <- exp(rnorm(num_iter, as.vector(rr1146$b), rr1146$se))

est1146 <- (quantile(rr1146.sim, probs = c(0.025,0.5, 0.975)))
est1146 <- as.data.frame(est1146)
est1146 <- t(est1146)

#study 905
dat.ratio905 <- subset(dat.ratio,dat.ratio$study.id==905)
rr905 <- rma(yi = dat.ratio905$lgrr,sei = dat.ratio905$se, method = "FE") 
rr905.exp <- predict(rr905, transf = exp, digits=4)
rr905.sim <- exp(rnorm(num_iter, as.vector(rr905$b), rr905$se))

est905 <- (quantile(rr905.sim, probs = c(0.025,0.5, 0.975)))
est905 <- as.data.frame(est905)
est905 <- t(est905)

#study 9911
dat.ratio9911 <- subset(dat.ratio,dat.ratio$study.id==9911)
rr9911 <- rma(yi = dat.ratio9911$lgrr,sei = dat.ratio9911$se, method = "FE") 
rr9911.exp <- predict(rr9911, transf = exp, digits=4)
rr9911.sim <- exp(rnorm(num_iter, as.vector(rr9911$b), rr9911$se))

est9911 <- (quantile(rr9911.sim, probs = c(0.025,0.5, 0.975)))
est9911 <- as.data.frame(est9911)
est9911 <- t(est9911)



comb.est <- rbind(est371, est671, est1131, est1146, est905, est9911)
comb.est <- as.data.frame(comb.est)
names(comb.est) <- c("ratio.ll","ratio.ce","ratio.ul")
comb.est$study.id <- c(371, 671, 1131, 1146, 905, 9911)

comb.est$lgrr <- logb(comb.est$ratio.ce)
comb.est$lgll <- logb(comb.est$ratio.ll)
comb.est$lgul <- logb(comb.est$ratio.ul)
comb.est$se <- (comb.est$lgul-comb.est$lgll)/(2*1.96)

dat.ratio.comb <- dat.ratio %>% 
  filter(study.id %in% c( 371, 671, 1131, 1146, 905, 9911)) %>% 
  distinct(study.id, .keep_all=T) %>% 
  select(-c(ratio.ll, ratio.ce, ratio.ul, lgrr, lgll, lgul, se))

dat.ratio.comb$outcome <- "CVD"

dat.ratio.comb <- merge(dat.ratio.comb, comb.est, by="study.id")

dat.ratio.unique <- dat.ratio %>% 
  filter(study.id %nin% c( 371, 671, 1131, 1146, 905, 9911)) %>% 
  filter(overall.estimate==1)


dat.ratio.overall <- rbind(dat.ratio.comb, dat.ratio.unique)
...
#dat.ratio.overall <- dat.ratio.overall[order(dat.ratio.overall$outcome2),]
...
forest(rr, xlim=c(-3, 2), at=log(c(0.5,1,2,3,3.5)), atransf=exp,
       ilab=dat.ratio.overall$pub.year,
       ilab.xpos=-1.5,cex=0.75, ylim=c(-1, 47),
...
text(-3,46, "Author(s)",  pos=4)
text(-1.5,46,"Year")
text(2.19,46, "Risk Ratio [95% CI]", pos=2)
...
subgroups <- rbind (rr.hosp, rr.mort, rr.mono, rr.coinf, rr.pre, rr.post, rr.lbias, rr.hbias)
...
levels(sPDF@data[["cat"]]) <- c('<2.3', '2.3 to 4.2', '4.3 to 5.6', '5.7 to 8.1', '8.2 to 11.6', '11.7 to 15.4', '15.5 to 22.4', '22.5 to 34.6', '34.7 to 62.3', '62.4 to 436')
...
levels(sPDF@data[["burdenCcat"]]) <- c('<2.3', '2.3 to 4.2', '4.3 to 5.6', '5.7 to 8.1', '8.2 to 11.6', '11.7 to 15.4', '15.5 to 22.4', '22.5 to 34.6', '34.7 to 62.3', '62.4 to 436')
